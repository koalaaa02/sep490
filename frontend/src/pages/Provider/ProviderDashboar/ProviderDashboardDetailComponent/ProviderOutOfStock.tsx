...
const ProviderOutOfStock = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${BASE_URL}/api/provider/statistics/product/OutOfStock?limit=5`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <>
      ...
      >
        <span className="fw-bold">Sản phẩm đã hết hàng</span>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>Sản phầm tồn kho đã hết</Modal.Title>
        </Modal.Header>
        <Modal.Body
          style={{
            maxHeight: "70vh",
          ...
          {errorMessage ? (
            ...
            <div>
              {inventoryData && inventoryData.length > 0 ? (
                ...
                <Alert variant="info">Không có hàng tồn kho đã hết</Alert>
              )}
            </div>
          )}
        </Modal.Body>
...
export default ProviderOutOfStock;
