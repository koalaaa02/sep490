...
public interface DebtPaymentMapper {
    ...
    @Mapping(source = "id", target = "id")
    List<DebtPaymentResponse> EntitiesToResponses(List<DebtPayment> debtPayment);
    
    @Mapping(source = "id", target = "id")
    DebtPayment RequestToEntity(DebtPaymentRequest debtPaymentRequest);
    
    @Mapping(source = "id", target = "id")
    List<DebtPayment> RequestsToEntities(List<DebtPaymentRequest> debtPaymentRequest);
}
